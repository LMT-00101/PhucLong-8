<?php
    class OrderModel{
        ...
        public function update_status_order($status, $order_id) {
            $order = pdo_query_one(
                "SELECT user_id, total, status FROM orders WHERE order_id = ?",
                $order_id
            );
            ...
            // status = 4: Giao thành công (hoàn thành đơn) — cộng điểm một lần
            $newStatus = (int) $status;
            if ($newStatus === 4 && $order && (int) $order['status'] !== 4) {
                require_once dirname(__DIR__, 2) . '/models/LoyaltyModel.php';
                $loyalty = new LoyaltyModel();
                $loyalty->addPoints((int) $order['user_id'], (int) $order_id, (int) $order['total']);
            }
        }
