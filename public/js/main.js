...
(function ($) {
    ...
    function decrementValue(e) {
        ...
        var parent = $(e.target).closest('.input-next-cart');
        var currentVal = parseInt(parent.find('.quantity-field-cart').val(), 10);
        var isCartPage = parent.closest('.shop-cart, .cart-page-section').length > 0;

        var newValue;
        if (isCartPage) {
            newValue = (!isNaN(currentVal) && currentVal > 1) ? currentVal - 1 : 1;
        } else {
            newValue = (!isNaN(currentVal) && currentVal > 0) ? currentVal - 1 : 0;
        }
    ...
    function openMinicart(e) {
        ...
        $('#minicart-overlay').addClass('active').fadeIn('fast');
        $('#shopping-cart').css('display', 'flex').hide().fadeIn('fast');
        $('body').css('overflow', 'hidden');
    }

    function closeMinicart() {
        $('#shopping-cart').fadeOut('fast');
        $('#minicart-overlay').fadeOut('fast', function () {
            $(this).removeClass('active');
        });
        $('body').css('overflow', '');
    }

    // Chỉ dùng minicart khi click đúng nút/minicart có id `cart`.
    // `#cart-mini` (giỏ hàng trên header) cần điều hướng tới trang giỏ đầy đủ.
    $('#cart').on('click', openMinicart);
    $('#close-minicart, #minicart-overlay').on('click', closeMinicart);
